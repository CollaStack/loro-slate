...
function applySlateOpToLoro(doc: LoroDoc, op: Operation): void {
  switch (op.type) {
    ...
    case "set_node": {
      ...
      const props = op.newProperties as Record<string, unknown>;
...
function getLoroNode(doc: LoroDoc, path: Path): LoroMap {
...
function getLoroText(doc: LoroDoc, path: Path): LoroText {
  return getLoroNode(doc, path).get("text") as unknown as LoroText;
}

function getLoroParentList(doc: LoroDoc, path: Path): LoroList {
...
function safeGet(map: LoroMap, key: string): unknown {
  try {
    return map.get(key);
  } catch {
    return undefined;
